...
def off_board(board, pos):
    ...
    return pos["x"] < 0 or pos["x"] >= width or pos["y"] < 0 or pos[
        "y"] >= height
...
class Battlesnake():
    ...
    def generate_next_board(board, move_map):
        ...
        for snake_id in move_map:
            ...
            snake = [
                s for s in next_board["board"]["snakes"] if s["id"] == snake_id
            ][0]
            ...
            if any(next_position == h for h in board["board"]["hazards"]):
                new_health = snake["health"] - board["game"]["ruleset"][
        ...
        snake_collided = {}
        for snake_id in [
                s["id"] for s in next_board["board"]["snakes"]
                if s["health"] > 0
        ]:
        ...
        living_snake_ids = [
            s["id"] for s in next_board["board"]["snakes"] if s["health"] > 0
        ]
        for snake_id in living_snake_ids:
            ...
            collided_with = [
                s for s in next_board["board"]["snakes"] if s["id"] != snake_id
                and s["health"] > 0 and snake["head"] == s["head"]
            ]
            if collided_with:
                # If the snakes are all the same length, they will all die
                if all([snake["length"] == s["length"]
                        for s in collided_with]):
        ...
        for snake_id in living_snake_ids:
            ...
            for other_snake_id in [
                    s for s in living_snake_ids if s != snake_id
            ]:
                other_snake = [
                    s for s in next_board["board"]["snakes"]
                    if s["id"] == other_snake_id
                ][0]
